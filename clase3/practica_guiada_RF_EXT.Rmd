...
metrics_dt <- class_metrics(decision_tree, truth = realiza_domest, estimate = .pred_class) %>% 
  mutate(modelo = "decision_tree")

metrics_bagging <- class_metrics(bagging, truth = realiza_domest, estimate = .pred_class) %>% 
...
Podemos ver que los valores más altos están asociados con un número de variables bajo y un número de muestras mínimas por partición más grande. ¿Cuál es el modelo que tiene mejor f1?

```{r}
show_best(tune_params, "f_meas")
```
...
```{r}
best_f1 <- select_best(tune_params, "f_meas")
...
metrics_random <- class_metrics(test_val, truth = realiza_domest, estimate = .pred_class) %>% 
