...
Hoy vamos a trabajar con K-medias y clustering jerárquico. Vamos a implementarlo con un set de datos ya conocido: un dataset (`.wb_bank_data_2019.csv`) con información construida por el Banco Mundial acerca de la distribución de la población ocupada según grandes sectores de actividad y categoría ocupacional:
...
1. Inicialización: Se comienza considerando que cada punto de datos es un cluster individual. En el principio, tenemos N clusters, donde N es el número de puntos de datos.

2. Cálculo de similitud: Se calcula la matriz de similitud o distancia entre todos los pares de clusters. Hay varias formas de medir la similitud, como la distancia euclidiana, la distancia de Manhattan, la similitud de coseno, etc.

3. Fusión de clusters: Se fusionan los dos clusters más similares en uno solo. La elección de cuáles clusters fusionar se basa en la matriz de similitud. Esto se repite en cada paso hasta que solo quede un cluster.

4. Construcción de la jerarquía: A medida que los clusters se fusionan, se construye una jerarquía en forma de árbol (dendrograma) que muestra cómo se agrupan los datos en diferentes niveles de similitud. En el dendrograma, los clusters se fusionan en función de la distancia en el eje vertical.

5. Selección de clusters: Para determinar el número óptimo de clusters, puede examinar el dendrograma y decidir en qué nivel de similitud desea cortar el árbol. Esto determinará el número de clusters.
