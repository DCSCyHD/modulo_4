...
final_fit <- readRDS('./models/final_bagging_model.rds')
...
Ahora, refresquemos el modelo de clase previa. Lo podemos importar como RDS.

```{r message=FALSE, warning=FALSE}
decision_tree_simple <- readRDS('./models/model_decision_tree.RDS')
...
```{r}
test_dt_simple <- decision_tree_simple %>%
  predict(test) %>%
  bind_cols(test, .)
...
Seleccionamos el mejor modelo en función de la métrica f1, y finalizamos el pipeline.
...
-   Captura un 5% de los casos positivos.
